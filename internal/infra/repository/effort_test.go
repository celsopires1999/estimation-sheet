...
func (s *EffortRepositoryTestSuite) TestEffort() {
	s.Run("should create a new effort", func() {
		...
		err := txm.Do(ctx, func(ctx context.Context, tx db.TransactionInterface) error {
			...
			s.Equal(competence.Code, createdEffort.CompetenceCode)
			s.Equal(competence.Name, createdEffort.CompetenceName)
	...
	s.Run("should update an effort", func() {
		...
		err := txm.Do(ctx, func(ctx context.Context, tx db.TransactionInterface) error {
			...
			s.Equal(competence.Code, updatedEffort.CompetenceCode)
			s.Equal(competence.Name, updatedEffort.CompetenceName)
